...
class TapListContract {
    ...
    data class TapListState(
        ...
        val filterTopOptions : SelectOption? = null,
        val filterOptions: FilterEntity? = null,
        val postsResult: ListEntity? = null,

        val selectedSortOption: String = "",

        val selectedSortTimeStart : Int? = 0,
        val selectedSortTimeEnd : Int? = 0,
        val selectedSortTime : String = "",
        ...
        val isValid: Boolean
            get() = selectedSortTime.isNotEmpty()
                || selectedMood.isNotEmpty()
                || selectedDogFriend.isNotEmpty()
                || selectedSafety.isNotEmpty()
                || selectedConvenience.isNotEmpty()
                || selectedEnvironment.isNotEmpty()
    }
